(function ($) {
    ...
    function kodory_form() {
        $('.showbtn').on('click', function (e) {
            $(this).parents('.kodory-form-toggle').siblings('.kodory-form-show').slideToggle();
            e.preventDefault();
        });
    }

    function kodory_view_mode_sidebar() {
    ...
    function kodory_view_mode_nosidebar() {
    ...
    function kodory_title_tooltip() {
        $(".kodory-tooltip").mousemove(function (event) {
            var relX = event.pageX - $(this).offset().left - $(this).children('.kodory-tooltip-inner').outerWidth() / 2;
            var relY = event.pageY - $(this).offset().top - $(this).children('.kodory-tooltip-inner').outerHeight() - 5;
            $(this).children('.kodory-tooltip-inner').css({"top": relY, "left": relX});
        });
    }

    function kodory_header_sticky($elem) {
        var $this = $elem;
        ...
        }).trigger('kodory_header_sticky');
        $(window).on('resize', function () {
            $this.trigger('kodory_header_sticky');
        });
    }

    function kodory_vertical_menu($elem) {
        /* SHOW ALL ITEM */
        var _countLi = 0,
    ...
    function kodory_auto_width_vertical_menu() {
    ...
    function kodory_init_carousel($elem) {
        $elem.not('.slick-initialized').each(function () {
            ...
            _this.on('init', function (event, slick, direction) {
                kodory_popover_button();
            });
            _this.slick(_config);
        });
    }

    function kodory_countdown($elem) {
        $elem.on('kodory_countdown', function () {
            $elem.each(function () {
                var _this = $(this),
                    _text_countdown = '';
        ...
        }).trigger('kodory_countdown');
    }

    // category product
    function kodory_category_product($elem) {
        $elem.each(function () {
            var _main = $(this);
    ...
    function kodory_magnific_popup() {
        ...
        $('.kodory-instagram-shop .item').magnificPopup({
            type: 'inline',
            mainClass: 'mfp-fade',
            removalDelay: 160,
            disableOn: false,
    ...
    function kodory_better_equal_elems($elem) {
        $elem.each(function () {
    ...
    function kodory_product_gallery($elem) {
        $elem.each(function () {
    ...
    function kodory_popover_button() {
    ...
    $(window).on("load", function () {
        kodory_video();
        kodory_form();
        kodory_view_mode_sidebar();
        kodory_view_mode_nosidebar();
        if ($('.owl-slick').length) {
            $('.owl-slick').each(function () {
                kodory_init_carousel($(this));
            });
        }
        if ($('.kodory-threed-view').length > 0) {
            kodory_threesixty();
        }
        if ($('.vertical-menu').length > 0) {
            kodory_auto_width_vertical_menu();
        }
        if ($('.kodory-countdown').length) {
            kodory_countdown($('.kodory-countdown'));
        }
        if ($('.product-gallery').length) {
            kodory_product_gallery($('.product-gallery'));
        }
        if ($('.block-nav-category').length) {
            kodory_vertical_menu($('.block-nav-category'));
        }
        if ($('.widget_kodory_nav_menu').length) {
            kodory_vertical_menu($('.widget_kodory_nav_menu'));
        }
        ...
        if ($('.widget_product_categories .product-categories').length) {
            kodory_category_product($('.widget_product_categories .product-categories'));
        }
        if ($('.header-sticky .header-wrap-stick').length) {
            kodory_header_sticky($('.header-sticky .header-wrap-stick'));
        }
        if ($('.equal-container.better-height').length) {
            kodory_better_equal_elems($('.equal-container.better-height'));
        }
        kodory_popover_button();
        kodory_magnific_popup();
        kodory_title_tooltip();
    });
    // Window resize
    $(window).on("resize", function () {
        if ($('.vertical-menu').length > 0) {
